...
class StatusBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final value = switch (status) {
      BookingStatus.draft => ('Draft', AppColors.textSecondary),
      BookingStatus.pendingPayment => ('Pending Payment', AppColors.warning),
      BookingStatus.confirmed => ('Confirmed', AppColors.success),
      BookingStatus.paymentFailed => ('Payment Failed', AppColors.error),
      BookingStatus.cancelled => ('Cancelled', AppColors.error),
      BookingStatus.completed => ('Completed', AppColors.info),
    };

    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
      decoration: BoxDecoration(
        color: value.$2.withValues(alpha: 0.12),
